...
class RandomRotationX(object):
    ...
    def __init__(self, degree,p=0.5,center=None):
        self.degree = degree
        self.p = p
    ...
    def __call__(self, img, mask):
        ...
        # angle = random.uniform(self.degrees[0], self.degrees[1])
            return F.rotate(img, self.degree, self.center), F.rotate(mask, self.degree, self.center)
        return img,mask
...
class RandomRotationY(object):
    ...
    def __call__(self, img, mask):
        ...
        if degree == 0 :
            ...
            return F.rotate(img, degree, self.center), F.rotate(mask, degree, self.center)
